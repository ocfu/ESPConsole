...
public:
   ...
   // Method to get a bool value by key
   bool getBool(const String &key, bool bDefault = false) {
      return configMap.count(key) ? (bool)configMap[key].toInt() : bDefault;
   }
   ...
   // Method to add a bool variable
   void addVariable(const String strName, bool bValue) {
      configMap[strName] = String(bValue);
      rebuildConfigString();
   }
