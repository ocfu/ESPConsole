  ...
  private:
   ...
   void tokenize() {
      ...
      while (*current && _nCount < MAX_TOKENS) {
         ...
         // Store current pointer as the start of token string
         _aszTokens[_nCount] = current;

         int foundDelimIdx = -1;
         char* foundDelimPos = nullptr;
         while (*current) {
            // don't split inside quotes, just continue
            if (inQuotes) {
               if (*current == '\"') {
                  inQuotes = false;
               }
               ...
               // we are not in quotes, check for delimiters to split for a token
