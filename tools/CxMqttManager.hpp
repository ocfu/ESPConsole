...
#ifdef DEBUG_BUILD
#define debug(...) _consoleInstance->debug(__VA_ARGS__)
#else
#define debug(...) ((void)0)
#endif
...
private:
   ...
   void _resubscribeTopics() {
      debug(F("re-subscribe all topics"));
      ...
      for (const auto& pair : _mapTopicCallbacks) {
         ...
         debug(F("re-subscribe topic %s"), strTopic.c_str());
   ...
   bool begin(const char* s = nullptr, int p = 0, int q = 0) {
      ...
      if (_strServer.length()) {
         ...
         _mqttClient.setCallback([this](const char* topic, uint8_t* payload, unsigned int length) {
            if (topic && payload) {
               payload[length] = '\0';
               debug(F("received from topic %s: '%s'"), topic, (char*)payload);

               for (const auto& pair : _mapTopicCallbacks) {
                  if (!pair.second.second) continue; // has no callback
                  if (_mapIsRelative[pair.first]) {
                     String strTopic = _strRootPath + '/' + pair.first;
                     debug(F("compare topics '%s' with '%s'"), strTopic.c_str(), topic);

                     if (strTopic == topic) pair.second.second(topic, payload, length);
                  } else {
                     debug(F("compare topics '%s' with '%s'"), pair.first+1, topic);
...
private:
   ...
   const char* _szName;       ///< Name of the topic (default: "noname").
...
public:
    ...
    */
   CxMqttTopicBase(const char* name, const char* topic , CxMqttManager::tCallback cb = nullptr, bool retain = false) : _bRetained(retain), _nQos(0), _strTopic(topic) {
      setName(name);
      setCb(cb);
   };
   ...
   void setName(const char* sz) {_szName = sz; if (!_szName) _szName = "noname";} ///< Sets the topic name.
   const char* getName() {return _szName;} ///< Retrieves the topic name.
...
public:
    ...
    */
   CxMqttTopic(const char* name, const char* topic, CxMqttManager::tCallback cb = nullptr, bool retain = false, bool autoSubscribe = true) : CxMqttTopicBase(name, topic, cb, retain) {if (autoSubscribe) subscribe();}
   CxMqttTopic(const char* topic, CxMqttManager::tCallback cb = nullptr, bool retain = false, bool autoSubscribe = true) : CxMqttTopic("", topic, cb, retain, autoSubscribe) {}
};
